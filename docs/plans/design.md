...
| **端口扫描 / 接口探测（自托管）** | **Instance Token 中间件拦截，无有效 Token 的请求直接 403，不透露接口存在** |
| **未授权初始化（自托管）** | **初始化接口同样需要 Instance Token，防止抢先注册攻击** |

### 2.5 自托管实例安全（Instance Token）

自托管版本面临与云端版本不同的威胁：任何人都可以扫描到服务端口并尝试访问 API。
解决方案是在**所有 API 端点前**部署一个 Instance Token 中间件。
...
│  ┌─────────────────────────────────────────────────┐    │
│  │  Instance Token 中间件                           │    │
│  │  检查 X-Phase-Instance-Token header              │    │
│  │                                                  │    │
│  │  ❌ Token 缺失或错误 → 403 Forbidden              │    │
│  │     （响应体为空，不透露 API 结构）                  │    │
│  │                                                  │    │
│  │  ✅ Token 正确 → 继续处理                          │    │
│  └─────────────────────────────────────────────────┘    │
...
**Instance Token 规格**：
- 部署时由管理员设置，存储在环境变量 `INSTANCE_TOKEN`
- 若未设置，服务器启动时自动生成并打印到日志
- 推荐长度：32 字节随机值，Base64 编码（约 44 字符）
- 客户端存储：首次验证成功后持久化到本地加密存储
- 每次 API 请求自动附加 `X-Phase-Instance-Token` header
...
用户（客户端）                          自托管服务器
      │                                       │
      │  输入服务器 URL + Instance Token        │
      │                                       │
      │  GET /health                          │
      │  X-Phase-Instance-Token: <token>      │
      │ ─────────────────────────────────────►│
      │                                       │  验证 Token
      ...
      │  客户端派生 Encryption Key + Auth Hash  │
      │                                       │
      │  POST /auth/init                      │
      │  X-Phase-Instance-Token: <token>      │
      │  { authHash, encryptedVault }         │
      │ ─────────────────────────────────────►│
      │  { jwtToken }                         │
      │ ◄───────────────────────────────────  │
      │                                       │
...
| 维度 | 云端版本 | 自托管版本 |
|------|---------|---------|
| 身份标识 | 邮箱 | Instance Token（单用户，无邮箱） |
| 账号注册 | 需要（邮箱+密码） | 不需要（开箱即用） |
| 防止未授权访问 | 服务器由官方控制 | Instance Token 中间件 |
| 多用户 | ✅ 支持 | ❌ 单用户设计 |
| 加密密钥派生 salt | email | 固定值 `"phase-selfhosted"`（或服务器生成的随机 salt） |
...
> **注意**：自托管版本所有端点（包括 `/health`）都需要携带
> `X-Phase-Instance-Token` header，否则返回 `403 Forbidden`。
> 云端版本无此 header 要求（服务器由官方控制，公开可访问）。
...
自托管版本为单用户设计，无邮箱概念，用 Instance Token 标识实例归属。

```
POST   /api/v1/auth/init
  描述: 首次初始化 vault（仅在 initialized=false 时可用，之后永久禁用）
  请求: { authHash: string, encryptedVault: string }
  响应: { token: string }
  前提: X-Phase-Instance-Token 验证通过 + initialized=false
  说明: authHash 的 PBKDF2 salt 使用服务器生成的随机 instanceSalt（由 /health 返回）

POST   /api/v1/auth/unlock
  描述: 解锁 vault（等同于登录）
  请求: { authHash: string }
  响应: { token: string }
  前提: X-Phase-Instance-Token 验证通过 + initialized=true

POST   /api/v1/auth/change-password
  描述: 修改主密码（自托管版本复用此端点）
  请求: { currentAuthHash: string, newAuthHash: string, encryptedVault: string, vaultVersion: number }
  响应: { success: true }
  认证: 需要 Bearer Token + X-Phase-Instance-Token
```
...
GET    /api/v1/health
  描述: 健康检查 + 实例状态
  响应 (云端):      { status: "ok", version: string }
  响应 (自托管):    { status: "ok", initialized: boolean, version: string, instanceSalt: string }
  说明:
    - initialized: 是否已完成初始化（有 vault），客户端据此决定显示 init 还是 unlock 界面
    - instanceSalt: 服务器首次启动时生成的随机 salt（Base64），用于客户端的 PBKDF2 密钥派生
      （替代云端版本中使用 email 作为 salt 的设计）
    - 自托管版本此端点同样需要 X-Phase-Instance-Token，确保端口扫描者无法探测实例信息
```
...
5. **自动生成以下环境变量并设置**：
   ...
   - `INSTANCE_TOKEN` — 实例访问令牌（32 字节随机值）
   - `INSTANCE_SALT` — PBKDF2 密钥派生 salt（32 字节随机值）
6. 部署完成页面**显示 `INSTANCE_TOKEN`**，用户复制后用于客户端首次连接
   （仅显示一次，之后只能通过 Cloudflare Dashboard 查看）
7. 返回 Worker URL

**一键部署 UI 设计要点**：
- 部署完成后的成功页面突出显示 `Instance Token`，并提示"请妥善保管，这是连接你的服务器的唯一凭证"
- 提供"复制"按钮和 QR 码（编码为 `phase://connect?url=<url>&token=<token>`），方便手机端扫码直接连接
